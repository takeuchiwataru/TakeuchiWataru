...
void CGrass::Update(void)
{
	...
	{// 武器が草に当たったら
		...
		{// パーティクルの生成
			CParticle::Create(D3DXVECTOR3(m_pos.x, m_pos.y + 40, m_pos.z), D3DXVECTOR3(sinf(rand() % 628 / 100.0f) * (rand() % 7 + 1), cosf(rand() % 628 / 100.0f) * (rand() % 7 + 1), 0.0f), G_PARTICLE_LIFE, G_PARTICLE_RADIUS,CParticle::PARTICLETYPE_000);
		}
		Uninit();
	}
}
