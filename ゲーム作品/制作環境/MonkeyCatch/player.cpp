...
void CPlayer::Update(void)
{
	...
	{// モードがゲーム
		...
		if (m_move.x < 0.2f && m_move.x > -0.2f && m_move.z < 0.2f && m_move.z > -0.2f && m_bJump == false)
		{// ニュートラル
		...
		{// 移動
			...
			{// パーティクルの生成
				CParticle::Create(D3DXVECTOR3(m_pos.x, m_pos.y + 5, m_pos.z), D3DXVECTOR3(sinf(rand() % 628 / 100.0f) * 0.5f, cosf(rand() % 628 / 100.0f) * 0.5f, 0.0f), P_PARTICLE_LIFE, P_PARTICLE_RADIUS, CParticle::PARTICLETYPE_001);
			}
		}
		if (m_bJump == true)
		{// ジャンプ
		...
		while (pScene != NULL)
		{
			...
			{// 死亡フラグが立っていたら
				...
				{// アイテムだったら
					((CItem*)pScene)->Collision(&m_pos, &m_posold, &m_move, &m_VtxMax, &m_VtxMin);
				}
...
void CPlayer::Draw(void)
{
	...
	vecLight = D3DXVECTOR4(-0.22f, 0.4f, -0.2f, 0.0f);
	pos = D3DXVECTOR3(0.0f, 0.8f, 0.0f);
