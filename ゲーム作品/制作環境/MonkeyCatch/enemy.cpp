...
void CEnemy::Update(void)
{
	...
	if (mode == CManager::MODE_GAME)
	{
		...
		{// 捕まったら
			...
			{// パーティクルの生成
				CParticle::Create(D3DXVECTOR3(m_pos.x, m_pos.y + 40, m_pos.z), D3DXVECTOR3(sinf(rand() % 628 / 100.0f) * (rand() % 7 + 1), cosf(rand() % 628 / 100.0f) * (rand() % 7 + 1), 0.0f), E_PARTICLE_LIFE, E_PARTICLE_RADIUS, CParticle::PARTICLETYPE_000);
			}
...
bool CEnemy::Collision(D3DXVECTOR3 *pPos, D3DXVECTOR3 *pPosold, D3DXVECTOR3 *pMove, D3DXVECTOR3 *pVtxMax, D3DXVECTOR3 *pVtxMin)
{
	...
	{// モデルの高さ分
		...
		if (pPos->z + pVtxMax->z > m_pos.z + m_VtxMin.z
			&& pPos->z + pVtxMin->z < m_pos.z + m_VtxMax.z)
		{// X面の判定
