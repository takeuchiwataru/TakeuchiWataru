...
CImage * CImage::Create(D3DXVECTOR3 pos, IMAGE type)
{
	...
	if (pFrame == NULL)
	{
		...
		if (pFrame != NULL)
		{
			...
			if (type == IMAGE_REMAIN)
			{// サルのカウント
				pFrame->BindTexture(CTexture::GetTexture(2));
			}
			if (type == IMAGE_NOKORI)
			{// 残り
				pFrame->BindTexture(CTexture::GetTexture(3));
			}
			if (type == IMAGE_TUTORIAL)
			{// コントローラー説明
				pFrame->BindTexture(CTexture::GetTexture(4));
			}
			if (type == IMAGE_CATCHNUM)
			{// 捕まえた数
				pFrame->BindTexture(CTexture::GetTexture(5));
			}
			if (type == IMAGE_REMAINTIME)
			{// 残り時間
				pFrame->BindTexture(CTexture::GetTexture(6));
			}
			if (type == IMAGE_TOTAL)
			{// 合計
				pFrame->BindTexture(CTexture::GetTexture(7));
			}
