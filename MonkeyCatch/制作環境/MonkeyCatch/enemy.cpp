...
CShadowX * CEnemy::m_pShadow = NULL;
CCircle * CEnemy::m_pCircle = NULL;
...
void CEnemy::Update(void)
{
	...
	if (mode == CManager::MODE_GAME)
	{
		...
		//// 草の取得
		//CGrass * pGrass = CGame::GetGrass();
		//D3DXVECTOR3 GrassPos = pGrass->GetPos();
		...
		{// お試し用
			CDebugProc::Print(1, "敵 : X %.1f, Y %.1f, Z %.1f\n", m_pos.x, m_pos.y, m_pos.z);
			CDebugProc::Print(1, "プレイヤーと敵の距離 : X %.1f, Y %.1f, Z %.1f\n", m_pos.x - pPlayer->GetPos().x, m_pos.y - pPlayer->GetPos().y, m_pos.z - pPlayer->GetPos().z);
			...
			//float fPosX = m_pos.x - pPlayer->GetPos().x;
			//float fPosZ = m_pos.z - pPlayer->GetPos().z;
			//if (fPosX <= 100.0f)
			//{// 
			//	m_move.x += ENEMY_MOVE000;
			//	CDebugProc::Print(1, "入った\n");
			//}
			//if (fPosX >= -100.0f)
			//{// 
			//	m_move.x -= ENEMY_MOVE000;
				...
				for (int nCntModel = 0; nCntModel < m_nNumParts; nCntModel++)
				{
					...
					{// パーツ情報
						...
						{// 親子関係
							...
							pStrCur = ReadLine(pFile, &aLine[0]);	// 1行読み込み
							pStrCur = GetLineTop(&aLine[0]);
							strcpy(&aStr[0], pStrCur);
						}
...
void CEnemy::LoadSetEnemy(void)
{
	...
	char aStr[MAX_CHAR];				// 文字列抜き出し用
	int nWord;
	D3DXVECTOR3 pos;					// テキストの位置保管用
	ENEMYTYPE type;						// テキストの敵の種類保管

	// ファイルオープン
	pFile = fopen(SETENEMY_FILE, "r");
	...
	{// ファイルが開けたら
		while (1)
		{
			...
			{// SCRIPTの文字があったら
				...
				if (memcmp(&aStr[0], MAX_ENEMY, strlen(MAX_ENEMY)) == 0)
				{// 敵の数
					pStrCur += strlen(MAX_ENEMY);	// 頭出し
					strcpy(&aStr[0], pStrCur);		// 対象の文字列抜き出し
					m_nNumEnemy = atoi(pStrCur);	// 文字列を値に変換

					pStrCur = ReadLine(pFile, &aLine[0]);	// 1行読み込み
					pStrCur = GetLineTop(&aLine[0]);
					strcpy(&aStr[0], pStrCur);

					if (memcmp(&aStr[0], TEXT_FILENAME, strlen(TEXT_FILENAME)) == 0)
					{// テキストファイル
						pStrCur = ReadLine(pFile, &aLine[0]);	// 1行読み込み
						pStrCur = GetLineTop(&aLine[0]);
						strcpy(&aStr[0], pStrCur);
						...
						{// テキストに書かれてる敵の数分だけ繰り返し
							if (memcmp(&aStr[0], ENEMYSET, strlen(ENEMYSET)) == 0)
							{// ENEMYSETの文字があったら
								pStrCur = ReadLine(pFile, &aLine[0]);	// 1行読み込み
								pStrCur = GetLineTop(&aLine[0]);
								strcpy(&aStr[0], pStrCur);
								...
								{// 位置
									...
									pos.x = (float)atof(pStrCur);	// X

									nWord = PopString(pStrCur, &aStr[0]);

									pStrCur += nWord;	// 次まで進める

									pos.y = (float)atof(pStrCur);	// Y

									nWord = PopString(pStrCur, &aStr[0]);

									pStrCur += nWord;	// 次まで進める

									pos.z = (float)atof(pStrCur);	// Z

									pStrCur = ReadLine(pFile, &aLine[0]);	// 1行読み込み
									pStrCur = GetLineTop(&aLine[0]);
									strcpy(&aStr[0], pStrCur);
								}
								if (memcmp(&aStr[0], TYPE, strlen(TYPE)) == 0)
								{// 種類
									pStrCur += strlen(TYPE);		// 頭出し
									strcpy(&aStr[0], pStrCur);

									type = (ENEMYTYPE)atoi(pStrCur);	
							...
							if (memcmp(&aStr[0], END_ENEMYSET, strlen(END_ENEMYSET)) == 0)
