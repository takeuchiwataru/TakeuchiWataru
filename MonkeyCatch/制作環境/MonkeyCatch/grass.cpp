...
//// 位置取得
////=============================================================================
//D3DXVECTOR3 CGrass::GetPos(void)
//{
//	return m_pos;
//	//return 	m_aTextpos[0];
//}
...
// 位置取得
//=============================================================================
D3DXVECTOR3 CGrass::GetPos(D3DXVECTOR3 *pPos)
{
	return *pPos;
	//return 	m_aTextpos[0];
}
...
void CGrass::LoadSetGrass(void)
{
	...
	GRASSTYPE type;						// テキストの敵の種類保管

	// ファイルオープン
	pFile = fopen(SETGRASS_FILE, "r");
	...
	{// ファイルが開けたら
		while (1)
		{
			...
			{// SCRIPTの文字があったら
				...
				if (memcmp(&aStr[0], MAX_GRASS, strlen(MAX_GRASS)) == 0)
				{// 草の数
					pStrCur += strlen(MAX_GRASS);	// 頭出し
					strcpy(&aStr[0], pStrCur);		// 対象の文字列抜き出し
					m_nNumGrass = atoi(pStrCur);	// 文字列を値に変換
					...
					{// テキストファイル
						...
						for (int nCnt = 0; nCnt < m_nNumGrass; nCnt++)
						{// テキストに書かれてる草の数分だけ繰り返し
							if (memcmp(&aStr[0], GRASSSET, strlen(GRASSSET)) == 0)
							{// GRASSSETの文字があったら
								pStrCur = ReadLine(pFile, &aLine[0]);	// 1行読み込み
								pStrCur = GetLineTop(&aLine[0]);
								strcpy(&aStr[0], pStrCur);
							...
							if (memcmp(&aStr[0], END_GRASSSET, strlen(END_GRASSSET)) == 0)
							{// END_GARSSSETの文字があったら
								pStrCur = ReadLine(pFile, &aLine[0]);	// 1行読み込み
								pStrCur = GetLineTop(&aLine[0]);
								strcpy(&aStr[0], pStrCur);
